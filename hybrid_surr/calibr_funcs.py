...
def switch_seir(
    ...
    if method == "expanding":
        beta = sim_data.iloc[:switch_day]["Beta"].expanding(1).mean().values[-1]
    elif method == "last":
        beta = sim_data.iloc[:switch_day]["Beta"].values[-1]

        S, E, I, R = seir_discrete.seir_model(
            y0, ts, beta, gamma, delta, stype="d", beta_t=False
        ).T
    elif method == "lstm":
        _, beta, _ = predict_Beta_I.predict_beta(
            ...
            sim_data,
            "lstm",
            np.arange(switch_day, modeling_duration),
            stochastic=False,
            count_stoch_line=0,
            sigma=0,
            gamma=0,
        ...
        S, E, I, R = seir_discrete.seir_model(
            y0, ts, beta, gamma, delta, stype="d", beta_t=True
        ).T
    ...
    fin = pd.concat([sim_data.iloc[:switch_day], fin]).reset_index(drop=True)
    return fin
...
def simulation_func(
    ...
    modeling_duration = np.array(modeling_duration).flatten()[
        0
    ]  # np.array(modeling_duration).flatten()[0]
    ...
    network_model = SEIRNetworkModel(n_nodes, network_type, chosen_seed)
    init_inf_frac = 0.0001
    init_rec_frac = 1 - alpha

    all_results = []
    for run in range(np.array(num_runs).flatten()[0]):
        res, rt, ri = network_model.simulate(
            beta=tau,
            gamma=gamma,
            delta=delta,
        ...
        seed_df["incidence"] = (temp["E_1"] - temp["E_0"]) - (temp["S_0"] - temp["S_1"])
    ...
    if all_results:
        ...
        seed_df[["S", "E", "I", "R", "incidence"]] = seed_df[
            ["S", "E", "I", "R", "incidence"]
        ].round()

    # print(seed_df['incidence'].astype(int))
    res = seed_df["incidence"].fillna(0).astype(int).values[:modeling_duration]
    # res = stz(res)
...
class real_G:
    ...
    def simulation_func_real(
        ...
        chosen_seed = np.random.RandomState(42)
        network_model = SEIRNetworkModel(n_nodes, network_type, chosen_seed, G=G_data)
...
def plot_results(
    ...
    if len(abc_results) == 0:
        axes[0].text(
            0.5,
            0.5,
            f"No accepted parameter sets for {method_name}",
        ...
        param_names = abc_results.columns.drop(["distance", "trajectory"]).values

        scatter = axes[0].scatter(
            abc_results[param_names[0]],
        ...
        # 'true' parameters
        axes[0].scatter(
            true_p0, true_p1, alpha=0.9, color="OrangeRed", label="Observed", s=50
        )
        ...
        if len(hm_results):
            axes[0].set_xlim(
                hm_results[param_names[0]].quantile(0.2),
                hm_results[param_names[0]].quantile(0.8),
            )
            axes[0].set_ylim(
                hm_results[param_names[1]].quantile(0.2),
                hm_results[param_names[1]].quantile(0.8),
            )
        axes[0].legend()
        axes[0].grid()
        ...
        sns.histplot(
            ...
            alpha=0.4,
            color="RoyalBlue",
            bins=min(abc_results.shape[0] + 1, 30),
            kde=True,
            stat="probability",
            edgecolor=None,
            ax=axes[2],
        ...
        axes[2].axvline(true_p0, ls="--", color="OrangeRed", label="Observed")
        ...
        axes[3].axvline(true_p1, ls="--", color="OrangeRed", label="Observed")
...
def posterior_params(gs, idata, observed_data, true_tau, true_alpha):
    ...
    # 'true' parameters
    ax_i.scatter(
        true_tau, true_alpha, alpha=0.9, color="OrangeRed", label="Observed", s=50
    )
    ...
    ax_i.axvline(true_tau, ls="--", color="OrangeRed", label="Observed")
    ...
    ax_i.axvline(true_alpha, ls="--", color="OrangeRed", label="Observed")
