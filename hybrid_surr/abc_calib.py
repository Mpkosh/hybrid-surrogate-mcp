...
def surr_sim(
    rng, alpha, beta, modeling_duration, n_nodes, top, koeff, shift, size=None
):
...
def calibr(
    draws=200, chains=4, epsilon=500, shift=[0], incidence=[], top=[False], koeff=[1]
):
    progressbar = True
    ...
    with pm.Model() as pm_modelp:
        ...
        alpha = pm.Uniform(name="alpha", lower=10 / n_nodes[0], upper=1.0)
        beta = pm.Uniform(name="tau", lower=0.0, upper=1.0)
        ...
        sim = pm.Simulator(
            "sim",
            surr_sim,
            params=(alpha, beta, modeling_duration, n_nodes, top, koeff, shift),
            epsilon=epsilon,
        ...
        end_time = time.time()
        idatap.extend(pm.sample_posterior_predictive(idatap, progressbar=progressbar))
    ...
    idata.to_netcdf(
        f"surr_on_real/sw_surr_{draws}_{chains}_eps{epsilon}_g{gamma[0]}_d{delta[0]}.nc"
    )
