...
# our functions
from hybrid_surr import choice_start_day, predict_Beta_I
...
def plot_one(
    ...
    predicted_Inc[0] = np.nan_to_num(predicted_Inc[0], neginf=0, posinf=0).astype(int)
    # predicted_Inc = predicted_Inc[:size-predicted_days[0]]
    # predicted_I[np.isnan(predicted_I)] = 0.0
    predicted_I[0] = np.nan_to_num(predicted_I[0], neginf=0, posinf=0).astype(int)
    ...
    rmse_I = rmse(
        np.nan_to_num(actual_I, neginf=0, posinf=0).astype(int), predicted_I[0]
    )
    actual_Inc = seed_df.iloc[predicted_days[0] :]["incidence"].values
    rmse_Inc = rmse(
        np.nan_to_num(actual_Inc, neginf=0, posinf=0).astype(int), predicted_Inc[0]
    )
    # print(rmse_Inc)
    # calc R^2
    r2 = r2_score(
        np.nan_to_num(actual_I, neginf=0, posinf=0).astype(int),
    ...
    actual_I_full = np.nan_to_num(actual_I_full, neginf=0, posinf=0).astype(int)
    actual_Inc_full = seed_df.iloc[:size]["incidence"].values
    actual_Inc_full = np.nan_to_num(actual_Inc_full, neginf=0, posinf=0).astype(int)

    predicted_I_full = [*actual_I_full[: predicted_days[0]], *predicted_I[0]]
    predicted_Inc_full = [*actual_Inc_full[: predicted_days[0]], *predicted_Inc[0]]
    r2_I_full = r2_score(actual_I_full, predicted_I_full)
    r2_Inc_full = r2_score(actual_Inc_full, predicted_Inc_full)
    ...
    if show_fig_flag:
        ...
        if len(beggining_beta) > 0:
            ...
            ax_b.plot(given_days, beggining_beta, color="green", ls="--", alpha=0.7)
        ax_b.plot(
            predicted_days,
            predicted_beta,
            color="green",
...
def main_f(
    ...
    for idx, seed_number in enumerate(seed_numbers):
        ...
        if switch_on_incidence:
            ...
            switch_col = "I"
        start_day = choice_start_day.choose_method(
            switch_col,
            seed_df,
            type_start_day,
            min_day=window_size,
        ...
        predicted_Inc = np.zeros((count_stoch_line + 1, predicted_days.shape[0]))
        ...
        ss, ee, predicted_I[0], _ = predict_Beta_I.predict_I(
            I_prediction_method,
            y,
        ...
        if on_incidence:
            ...
            predicted_Inc[0] = np.append(
                from_last, (ee[:-1] - ee[1:]) - (ss[1:] - ss[:-1])
            )
            predicted_Inc[0][predicted_Inc[0] < 0] = 0

        if stochastic:
            for i in range(count_stoch_line):
                ss, ee, predicted_I[i + 1], _ = predict_Beta_I.predict_I(
                    I_prediction_method,
                    y,
                    predicted_days - start_day,
                    predicted_beta,
        ...
        rmse_I, rmse_Inc, rmse_Beta, r2, r2_Inc, r2_I_full, r2_Inc_full, peak = (
            plot_one(
                ax_to_func,
                chosen_col,
                predicted_days,
...
def apply_methods(
    ...
    for type_start_day in types_start_day:
        ...
        for beta_pred, new_label in zip(methods[idx_s:idx_e], new_labels[idx_s:idx_e]):
