...
from arviz.stats.density_utils import _fast_kde_2d, _find_hdi_contours
...
def plots(
    ...
    with_switch, num_runs, frac, gamma, delta, n_nodes, top, koeff, shift = (
        network_params
    )
    ...
    if ax is None:
        fig, ax = plt.subplots(1, alpha_len, sharex=True, sharey=True, figsize=(6, 3))
    ...
    if pred:
        l0 = ax[i].plot(
            sim_part[:switchpoint, 0], color="gray", label="Simulation", alpha=0.5
        )
        if num_runs[0] == 0:
            l0 = ax[i].plot(sim_part[:switchpoint, 1::nth], color="gray", alpha=0.05)
        else:
            l0 = ax[i].plot(sim_part[:switchpoint, 1:], color="gray", alpha=0.05)

    a = time.time()
    # for surrogate plot each Nth, as there're 40k lines originally
    if num_runs[0] == 0:
        l00 = ax[i].plot(
            np.arange(switchpoint, fin_size),
    ...
    if not pred:
        if num_runs[0] > 0:
            for j in range(n_hyb_runs):
                # num_runs=1
                q = calibr_funcs.simulation_func(
                    1,
                    tau=[p0_mode],
                    alpha=[p1_mode],
            ...
            ax[i].plot(all_q[best_idx], color="white", lw=4, ls="-", zorder=980)
            l1 = ax[i].plot(
                all_q[best_idx],
                color="ForestGreen",
                lw=2,
                ls="-",
                label=r"Best simulation ($R^2$" + f" = {all_r[best_idx]:.3f})",
                zorder=990,
            )
            ...
            ax[i].plot(q, color="white", lw=4, ls="-", zorder=980)
            l1 = ax[i].plot(
                q,
                color="ForestGreen",
                lw=2,
                ls="-",
                label=r"Best simulation ($R^2$" + f" = {r2_part:.3f})",
                zorder=990,
            )
            best_r2 = r2_part
    ...
    if pred:
        ...
        l3 = ax[i].scatter(
            np.arange(switchpoint, data.shape[0]),
            data_part[switchpoint:],
            color="Grey",
            s=50,
            label="Unknown data",
        ...
        ax[i].axvline(
            switchpoint, ls="--", color="k", lw=1  # , label='Forecast starts'
        )
...
def calc_stat(idata, param_names):
    ...
    density, xmin, xmax, ymin, ymax = _fast_kde_2d(
        idata.posterior[param_names[0]],
        idata.posterior[param_names[1]],
        gridsize=gridsize,
    )
    hdi_probs = [0.2]
...
def results_calib(
    observed_data, idata, true_tau, true_alpha, network_params, method_name="ABC SMC"
):
    """
    ...
    plots(
        idata,
        observed_data["incidence"].values,
        "",
        ax=ax_i,
        p0_mode=p0_mode,
        p1_mode=p1_mode,
    ...
    for i, pname, fname, pval, pmode in zip(
        np.arange(2),
        param_names[::-1],
        fancy_names[::-1],
        ...
        ax_i.axvline(pval, ls="--", color="white", lw=4)
        ax_i.axvline(pval, ls="--", color="OrangeRed", lw=2, label="Observed")

        ax_i.axvline(pmode, ls="--", color="white", lw=4)
        ax_i.axvline(pmode, ls="--", color="green", lw=2, label="Chosen")
...
def pred_calib(
    ...
    for i in range(n_chains):
        ...
        label = ""
    # 'true' parameters
    ax_i.scatter(true_tau, true_alpha, alpha=0.9, color="white", s=60)
    ax_i.scatter(
        true_tau,
        true_alpha,
        alpha=0.9,
    ...
    # __ mode params
    ax_i.scatter(p0_mode, p1_mode, alpha=0.9, color="white", s=60)
    ax_i.scatter(
        p0_mode,
        p1_mode,
        alpha=0.9,
    ...
    for i, pname, fname, pval, pmode in zip(
        ...
        ax_i.axvline(pmode, ls="--", color="white", lw=4)
        ax_i.axvline(pmode, ls="--", color="green", lw=2, label="Mode")
        ax_i.set_title(fname + ", posterior distribution")
        ax_i.set_xlabel(fname)
        ax_i.legend()
        ax_i.grid()
...
def plot_calib(
    ...
    if not ax_curves:
        ...
        gs0 = gs[0].subgridspec(2, 1, height_ratios=[1, 4], hspace=0.0)
        gs1 = gs[1].subgridspec(
            2, 2, wspace=0, hspace=0.0, width_ratios=[5, 1], height_ratios=[1, 4]
        )
    ...
    q = az.plot_pair(
        ...
        kde_kwargs={
            ...
            "contourf_kwargs": {"alpha": 0.3, "colors": [colors_l[-1], *colors_l[:-1]]},
        },
        marginals=True,
        # point_estimate="mode",
    ...
    ax_scatter.set_xticks(
        np.arange(min_x, 1, 0.2),
        np.arange(min_x, 1, 0.2).round(1),
        # fontsize=flabel
    )
    ax_scatter.set_yticks(
        np.arange(min_y, 1, 0.1),
    ...
    ls3 = ax_scatter.scatter(
        true_tau, true_alpha, zorder=0, s=5, color=fc, label="Simulation"
    )

    ax_scatter.set_xlabel(
        fancy_names[0],  # fontsize=flabel
    )
    ax_scatter.set_ylabel(
        fancy_names[1],  # fontsize=flabel
    )
    ...
    for c, val in zip(colors_l[::-1], hdi_list):
        legend_elements.append(
            mpatches.Patch(color=c, label=f"HDR {int(val*100)}%", alpha=0.9)
        )
    ...
    if pred:
        ...
        flabel_p = 10
    ax_scatter.legend(handles=[ls1, ls2, ls3, *legend_elements], fontsize=10)
    ax_scatter.grid()
    """
