...
def create_peak_plot(folder_name='',observed_data = '',
                    ...
                    with_outliers=True, same_lims=False, 
                     figsize=(8,4), 
                     x_lim = (-130, 20), y_lim = (0.6, 2.3),
                     alpha_m=0.4, alpha_area=0.35,
                     save=False):
    ...
    for i,idata_ms,sub_labels in zip(np.arange(len(idatas)),
                                idatas,
                               [['Hybrid', 'Surrogate'] for i in range(len(idatas))]): 
        plot_peaks_ax(axes[i], idata_ms, sub_labels, folder_name, 
                      ph_actual,pt_actual,
        ...
        fontsize=9
        axes[i].annotate(labels[i], xy=(0.12, 1.35), 
    ...
    if save:
        plt.savefig(f'{folder_name}/peaks_area.pdf', 
...
def plot_peaks_ax(ax, idata_ms, sub_labels, 
                  folder_name, ph_actual,pt_actual,
                  x_lim = (-130, 20), y_lim = (0.6, 2.3), 
                  size_m=120, alpha_m=0.4, 
                  alpha_area=0.35, with_outliers=True):
    ...
    for label, idata_m in zip(sub_labels, idata_ms):
        try:
            ...
            col = colors.pop()
            ax.scatter(pt_pred,ph_pred, marker='.', s=size_m,  alpha=alpha_m, 
                       label=label, zorder=10, color=col)
            
            ax.fill(pt_pred.iloc[hull.vertices], 
...
def metric_hmaps(folder_name,fin, met, 
                 suff='', exclude=[], figsize=(15,10),
                size=15,save=False):
