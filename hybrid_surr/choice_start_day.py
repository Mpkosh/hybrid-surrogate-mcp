...
def choose_method(chosen_col, seed_df, start_day, frac=0.01, min_day=14, n_people=1000):
    if start_day == "fraq_people":
        start_day_v = cpoint_fraq_people(chosen_col, seed_df, n_people, min_day=min_day)
    elif start_day == "roll_var_npeople":
        start_day_v = cpoint_roll_var_npeople(
            chosen_col, seed_df, min_day=min_day, n_people=n_people
        )
    else:
...
def cpoint_fraq_people(chosen_col, seed_df, n_people, min_day=4):
...
def cpoint_roll_var_npeople(chosen_col, seed_df, thresh=0.1, n_people=100, min_day=10):
    scaler = MinMaxScaler()
