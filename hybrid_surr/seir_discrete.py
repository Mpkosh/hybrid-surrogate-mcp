...
def seir_model(y, predicted_days, beta, sigma, gamma, stype="stoch", beta_t=False):
    ...
    for time_stamp in predicted_days[:-1] - predicted_days[0]:
        ...
        if stype == "stoch":
            res[time_stamp + 1] = seir_one_day_stoch(
                res[time_stamp], beta_val, sigma, gamma
            )
        else:
            res[time_stamp + 1] = seir_one_day(res[time_stamp], beta_val, sigma, gamma)

        if np.sum(res[time_stamp + 1]) > full_pop:
            pass
