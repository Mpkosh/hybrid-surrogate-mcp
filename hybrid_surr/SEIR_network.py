...
class SEIRNetworkModel:
    def __init__(self, population: int, ntype: str, chosen_seed, G=0):
        ...
        if ntype == "ba":
            ...
            self.G = nx.fast_gnp_random_graph(
                population, 5 / population, seed=chosen_seed
            )
        elif ntype == "custom":
            self.G = G
            # print('r')
    ...
    def transform_event_times_to_days(
        self, model_output, tmax, I_frac_switch, frac_pop
    ):
        indices = []
        ...
        for day in range(max_day):
            ...
            if frac_pop == 'Infected':
                ...
                if inc > self.population*I_frac_switch:
                    break
            """
        new_model_output = SEIRModelOutput(
            model_output.t[indices],
    ...
    def simulate(
        ...
        assert (
            initial_recovered + initial_infected < self.population
        ...
        self.result = self.transform_event_times_to_days(
            seir_o, tmax, I_frac_switch, frac_pop
        )
