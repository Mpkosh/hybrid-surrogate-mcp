...
def test_calibrate_model_complete_data_3in1():
    ...
    assert alpha_modes == [pytest.approx(0.876, 0.01),
                           pytest.approx(0.894, 0.01),
                           pytest.approx(0.885, 0.01)]
    assert beta_modes == [pytest.approx(0.161, 0.01),
                        pytest.approx(0.149, 0.01),
                         pytest.approx(0.151, 0.01)]
    for i in best_r2s:
        assert i>0. # for THESE  examples 


def test_calibrate_model_forecast_3in1_hybrid():
    alpha_modes, beta_modes, fig_path = (
                service.calibrate_model_forecast_3in1(model_name='hybrid',
    ...
    assert alpha_modes == [pytest.approx(0.701, 0.01),
                           pytest.approx(0.742, 0.01),
                           pytest.approx(0.955, 0.01)]
    assert beta_modes == [pytest.approx(0.331, 0.01),
...
def test_calibrate_model_forecast_3in1_surrogate():
    alpha_modes, beta_modes, fig_path = (
                service.calibrate_model_forecast_3in1(model_name='surrogate',
    ...
    assert alpha_modes == [pytest.approx(0.736, 0.01),
                           pytest.approx(0.731, 0.01),
                           pytest.approx(0.935, 0.01)]
    assert beta_modes == [pytest.approx(0.277, 0.01),
